...
def get_shared_data_path():
    """Mendapatkan path untuk menyimpan data yang dibagikan antar pengguna"""
    return "shared_data.xlsx"

def save_shared_data(df_clustered):
    """Menyimpan data hasil klasterisasi untuk dibagikan"""
    try:
        file_path = get_shared_data_path()
        df_save = df_clustered.copy()
        # Format kehadiran untuk display
        if 'Kehadiran' in df_save.columns:
            df_save['Kehadiran'] = df_save['Kehadiran'].apply(lambda x: f"{x:.2%}" if isinstance(x, (int, float)) else x)
        df_save.to_excel(file_path, index=False)
        return True
    except Exception as e:
        st.error(f"Gagal menyimpan data bersama: {e}")
        return False

@st.cache_data
def load_shared_data():
    """Memuat data hasil klasterisasi yang dibagikan"""
    file_path = get_shared_data_path()
    if os.path.exists(file_path):
        try:
            ...
            st.error(f"Gagal memuat data bersama: {e}")
            return None
    return None

def generate_pdf_profil_siswa(nama, data_siswa_dict, klaster, cluster_desc_map):
    """Generate PDF profil siswa dengan error handling yang lebih baik"""
    try:
        pdf = FPDF()
        pdf.add_page()
        ...
        if isinstance(kehadiran_val, str) and '%' in kehadiran_val:
            kehadiran_display = kehadiran_val
        else:
            kehadiran_display = f"{float(kehadiran_val):.2%}"
            
        display_data = {
            "Nomor Induk": str(data_siswa_dict.get("No", "-")),
            "Jenis Kelamin": str(data_siswa_dict.get("JK", "-")),
            "Kelas": str(data_siswa_dict.get("Kelas", "-")),
            "Rata-rata Nilai Akademik": f"{float(data_siswa_dict.get('Rata Rata Nilai Akademik', 0)):.2f}",
            "Persentase Kehadiran": kehadiran_display,
...
def preprocess_data(df):
    """Preprocess data dengan error handling yang lebih baik"""
    try:
        ...
        st.error(f"Error dalam preprocessing data: {e}")
        return None, None

def run_kprototypes_clustering(df_preprocessed, n_clusters):
    """Run K-Prototypes clustering dengan error handling"""
    try:
...
def generate_cluster_descriptions(df_clustered, n_clusters, numeric_cols, categorical_cols):
    ...
    for i in range(n_clusters):
        ...
        mode_values = cluster_data[categorical_cols].mode()
...
def init_session_state():
    ...
    if 'categorical_features_indices' not in st.session_
...
def show_operator_tu_page():
    ...
    if st.session_state.current_menu == "Unggah Data":
        ...
        if st.session_state.df_preprocessed_for_clustering is None or st.session_state.df_preprocessed_for_clustering.empty:
            ...
            if st.button("Jalankan Klasterisasi"):
                ...
                if df_clustered is not None:
                    ...
                    # Simpan data untuk kepala sekolah
                    if save_shared_data(df_final):
                        st.success(f"Klasterisasi selesai dengan {k} klaster! Data berhasil disimpan untuk akses Kepala Sekolah.")
        ...
        if st.session_state.df_preprocessed_for_clustering is None or st.session_state.df_preprocessed_for_clustering.empty:
            ...
            karakteristik dari setiap kelompok siswa.
            </div>
            """, unsafe_allow_html=True)
            st.markdown("---")
            
            k_visual = st.slider("Jumlah Klaster (K) untuk visualisasi", 2, 6, value=st.session_state.n_clusters,
                                 help="Geser untuk memilih jumlah klaster yang ingin Anda visualisasikan.")
            ...
            if df_for_visual_clustering is not None:
                ...
                for i in range(k_visual):
                    ...
                    if cluster_data.empty:
                        st.warning(f"Klaster {i} tidak memiliki data.")
                        continue
                    ...
                    with col2:
                        ...
                        try:
                            ...
                            mode_ekskul = cluster_data[CATEGORICAL_COLS].mode()
                            if not mode_ekskul.empty:
                                values_for_plot_ekskul = [int(mode_ekskul.iloc[0][col]) for col in CATEGORICAL_COLS]
                            else:
                                values_for_plot_ekskul = [0] * len(CATEGORICAL_COLS)
                            ...
                            y_min = min(values_for_plot) - 0.2 if values_for_plot else -1
                            y_max = max(values_for_plot) + 0.2 if values_for_plot else 1
                            ax.set_ylim(y_min, y_max)
                            
                            for index, value in enumerate(values_for_plot):
                                offset = 0.05 if value >= 0 else -0.1
                                ax.text(bars.patches[index].get_x() + bars.patches[index].get_width() / 2, 
                                       bars.patches[index].get_height() + offset, 
                                       f"{value:.2f}", ha='center', fontsize=9, weight='bold')
                            ...
                            st.error(f"Error dalam membuat visualisasi: {e}")
        ...
        if st.session_state.df_clustered is None or st.session_state.df_original is None or st.session_state.df_original.empty:
            ...
            if nama_terpilih:
                ...
                klaster_desc_for_student = st.session_state.cluster_characteristics_map.get(klaster_siswa_terpilih, "Deskripsi klaster tidak tersedia.")
                st.markdown(f"""
                ...
                with col_info:
                    ...
                    st.markdown(f"Rata-rata Nilai Akademik: {siswa_data.get('Rata Rata Nilai Akademik', 0):.2f}")
                    st.markdown(f"Persentase Kehadiran: {siswa_data.get('Kehadiran', 0):.2%}")
                ...
                with col_chart:
                    ...
                    try:
                        ...
                        for bar, val in zip(bars.patches, values_siswa_plot):
                            ax.text(bar.get_x() + bar.get_width() / 2, val + (ax.get_ylim()[1] * 0.02), 
                                   f"{val:.1f}", ha='center', fontsize=9, weight='bold')
                        ...
                        st.error(f"Error dalam membuat visualisasi profil siswa: {e}")
                ...
                if st.session_state.cluster_characteristics_map:
                    if st.button("Generate & Unduh Laporan PDF", key="unduh_pdf_tu"):
                    ...
                    st.warning("Mohon lakukan klasterisasi terlebih dahulu untuk menghasilkan data profil PDF.")


def show_kepala_sekolah_page():
    """Halaman untuk Kepala Sekolah"""
    # Load shared data
    df_kepsek_load = load_shared_data()
    ...
    if df_kepsek_load is not None:
        ...
        if 'df_original' not in st.session_state or st.session_state.df_original is None:
            ...
            # Convert percentage string back to float if needed
            ...
            # Generate cluster descriptions if not exist
            df_preprocessed, scaler = preprocess_data(st.session_state.df_original)
    ...
    if st.session_state.df_clustered is None or st.session_state.df_clustered.empty:
        st.warning("Data hasil klasterisasi tidak ditemukan. Mohon minta Operator TU untuk memproses dan menyimpan hasilnya terlebih dahulu.")
        return
    ...
    if st.session_state.kepsek_current_menu == "Lihat Hasil Klasterisasi":
        ...
        for i in range(st.session_state.n_clusters):
            ...
            with col1:
                ...
                # Generate normalized data for display
                ...
                if df_preprocessed_temp is not None:
                    ...
                    if not cluster_data_norm.empty:
                        st.write("Rata-rata Nilai & Kehadiran (Dinormalisasi):")
                ...
                mode_ekskul = cluster_data[CATEGORICAL_COLS].mode()
                if not mode_ekskul.empty:
                    mode_ekskul_display = mode_ekskul.iloc[0].apply(lambda x: 'Ya' if str(x) == '1' else 'Tidak')
                    st.dataframe(mode_ekskul_display.to_frame(name='Paling Umum'), use_container_width=True)
            ...
            with col2:
                ...
                try:
                    if df_preprocessed_temp is not None and not cluster_data_norm.empty:
                        values_for_plot_numeric = cluster_data_norm[NUMERIC_COLS].mean().tolist()
                        mode_ekskul = cluster_data_norm[CATEGORICAL_COLS].mode()
                        if not mode_ekskul.empty:
                            values_for_plot_ekskul = [int(mode_ekskul.iloc[0][col]) for col in CATEGORICAL_COLS]
                        else:
                            values_for_plot_ekskul = [0] * len(CATEGORICAL_COLS)
                        ...
                        st.warning("Data tidak dapat divisualisasikan.")
        ...
        st.header("Lihat Profil Siswa Berdasarkan Nama")
        st.info("Pilih nama siswa dari daftar di bawah untuk melihat detail profil mereka.")
        st.markdown("---")
        ...
        if nama_terpilih_kepsek:
            ...
            with col_info:
                ...
                # Handle kehadiran display
                kehadiran_val = siswa_data.get('Kehadiran', '0%')
                if isinstance(kehadiran_val, str) and '%' in kehadiran_val:
                    st.markdown(f"Persentase Kehadiran: {kehadiran_val}")
                else:
                    st.markdown(f"Persentase Kehadiran: {float(kehadiran_val):.2%}")
            ...
            with col_chart:
                ...
                try:
                    ...
                    nilai_akademik = siswa_data.get("Rata Rata Nilai Akademik", 0)
                    kehadiran_val = siswa_data.get("Kehadiran", "0%")
                    if isinstance(kehadiran_val, str) and '%' in kehadiran_val:
                        kehadiran_pct = float(kehadiran_val.replace('%', ''))
                    else:
                        kehadiran_pct = float(kehadiran_val) * 100
                    
                    values_siswa_plot_numeric = [nilai_akademik, kehadiran_pct]
                    values_siswa_plot_ekskul = [
            ...
            if st.session_state.cluster_characteristics_map:
                if st.button("Generate & Unduh Laporan PDF", key="unduh_pdf_kepsek"):
                    with st.spinner("Menyiapkan laporan PDF..."):
                        siswa_data_for_pdf = siswa_data.drop(labels=["Klaster"]).to_dict()
                        
                        # Convert percentage string back to float for PDF
                        if isinstance(siswa_data_for_pdf.get('Kehadiran'), str) and '%' in siswa_data_for_pdf['Kehadiran']:
